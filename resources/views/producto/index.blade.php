                ...
                @method("GET")
                    <div class="row">
                            <div class="col-12 col-md-4 form-box">
                                <input class="form-control " type="search" placeholder="Nombre" name="nombre" aria-label="nombre" value="{{$request->get('nombre')}}">
                            </div>
                            <div class="col-12 col-md-4 form-box">
                                <input class="form-control " type="search" placeholder="Categoria" name="category" aria-label="category" value="{{$request->get('category')}}">
                            </div>
                            <div class="col-12 col-md-4 form-box">
                                <input class="form-control " type="number" placeholder="Precio" name="precio" aria-label="precio" value="{{number_format($request->get('precio'),2)}}">
                            </div>
                            <div class="col-12 col-md-4 form-box">
                                <select name="proveedor" class="form-control" id="proveedor">
                                        <option value="">Selecciona un proveedor</option>
                                    @foreach ($proveedores as $proveedor)
                                        <option value="{{$proveedor->_id}}" {{ ($proveedor->_id == $request->get('proveedor')) ? 'selected' : '' }}>{{$proveedor->nombre}}</option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <tbody>
                    @foreach($productos as $producto)
                        <tr>
                            ...
                            <td class="text-center"><textarea  class="form-control" readonly cols="30" rows="4">{{$producto->descripcion}}</textarea></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
<script type="module" >
    ...
    import { fetchDelete } from '{{asset("resources/js/globals.js")}}';
